...
//			}
	...
	*/
	...
	function setRepere($titre, $posX, $posY, $sizeX, $sizeY, $datasX, $datasY, $droites)
	{
		...
		// Si le min=max alors on change le nombre de d?upage d'ordonn?
		...
		switch (count($datasX))
		{
			...
			break;
			case 2:
				// Mettre les deux valeurs en d?t et fin d'axe	
		...
		if ($datasY[2] == 5) //**** minY et maxY diff?nts ****//
			for ($i=0 ; $i <= $datasY[2] ; $i++)
			{
				// Mettre les $i valeurs entre le d?t et la fin de l'axe
		...
		else //**** minY et maxY ?ux --> 1 ou 2 intervalles au lieu de 5
		{
...
//			$l=4*$i+4;
			...
			// ajouter la l?nde si elle doit ?e
			if ($droites[$i][3] != "")
			{
	...
	//***********************************************************************************************************
	// Pour ?ire un texte dans ue case... [BUI] pour le style de la police et [[LCR]] pour le centrage ?ntuel
	// Par d?ult, le texte sera normal et ?auche...
	// Fonction destin??essiner un tableau dans un file.pdf
	function drawTableau(&$pdf, $tableType, $headerType, $headerDatas, $datasType, $datas)
	{
		...
		for($i=0; $i<$nbCol; $i++) 
		{
			...
			// Si une donn?== 0 alors on affiche rien...
			if ($header_type[$i]['TEXT'] != "0") ;
			else $header_type[$i]['TEXT'] = "";
			
			// par d?ut, le texte est centr? gauche, non italic, non soulign?t non gras.
			// par d?ut, les cellules ne sont pas fusionn?.
			$header_type[$i]['T_TYPE'] = '';
			$header_type[$i]['T_ALIGN'] = '';		
			$header_type[$i]['COLSPAN'] = "1";
		}

		// Si l'utilisateur veut un alignement sp?fique pour la premi? colonne. Sinon, T_ALIGN  prend le dessus...
		if (isset($headerType['T_ALIGN_COL0']))
			$header_type[0]['T_ALIGN'] = $headerType['T_ALIGN_COL0'];

		// Si l'utilisateur veut un fond color?p?fique  pour la premi? colonne. Sinon, BG_COLOR  prend le dessus...
		if (isset($headerType['BG_COLOR_COL0']))
			$header_type[0]['BG_COLOR'] = $headerType['BG_COLOR_COL0'];
				
		// Si l'utilisateur pr?se un type ou un alignement pour une cellule pr?se du tableau, on l'applique ici
		...
		for($i=0; $i<$nbCol; $i++) 
		{
			if (sscanf($header_type[$i]['TEXT'], "[%[a-zA-Z]]%s", $balise, $reste) != 0)
			{
				...
				else
				{
					...
					// On teste les diff?ntes balises pour ajuster la cellule.
		...
		// Test si l'utilisateur veut fusionner DEUX cellules dans le header de son tableau. Il doit mettre "COLSPAN2" dans la premi? cellule ?usionner.
		for($i=0 ; $i<$nbCol ; $i++)
		{
			$k=$nbCol+$i;
			$i_1 = $i-1;
		...
		{
			//*********************************************************************
			// Ce qui suit est valable pour la premi? colonne du tableau
			//*********************************************************************
			// si l'utilisateur a pr?s?n alignement pour la premi? colonne, on l'applique ici
			if (isset($datasType['T_ALIGN_COL0']))
				$data[0]['T_ALIGN'] = $datasType['T_ALIGN_COL0'];
				
			// Si l'utilisateur a pr?s?ne couleur de fond pour la premi? colonne, on l'applique ici.
			...
			for ($j=$i ; $j<$i+$nbCol ; $j++)
			{
				...
				// Si l'utilisateur a pr?s?ne couleur de fond pour les autres colonnes, on l'applique ici.
				...
				// Si la valeur de la cellule est 0, le choix a ? fait ICI de ne rien mettre dans la cellule.
				if ($data[$k]['TEXT'] == "0")
					$data[$k]['TEXT'] ="";
					
				// Test si l'utilisateur veut fusionner deux cellules dans le header de son tableau. Il doit mettre le contenu
				// de la cellule fusionn?dans la premi? cellule et "COLSPAN2" dans la deuxi? cellule.
